...
if (!$entity instanceof ElggEntity) {
	elgg_log("object/elements/card expects an ElggEntity in \$vars['entity']", 'ERROR');
	return;
}
...
if (empty($social) && $social !== false && $entity instanceof ElggEntity) {
	$vars['social'] = elgg_view_menu('entity:social', [
		'entity' => $entity,
		'sort_by' => 'priority',
		'class' => 'elgg-menu-hz',
	]);
...
if (empty($media) && $media !== false) {
	if ($entity->hasIcon('large')) {
		$vars['media'] = elgg_view_entity_icon($entity, 'master');
	}
}

$media = elgg_view('object/elements/card/media', $vars);
unset($vars['media']);

$header = elgg_view('object/elements/card/header', $vars);
$body = elgg_view('object/elements/card/body', $vars);
$footer = elgg_view('object/elements/card/footer', $vars);
