...
if (array_key_exists('value', $vars)) {
	$value = $vars['value'];
} else if ($value = get_input('q', get_input('tag', null))) {
	$value = $value;
}
...
<span class="search_form">Search</span>
	<div class ="search_btn">
<form action="<?php echo elgg_get_site_url(); ?>search" method="get">

    <div class="field">
        <p class="control has-icons-left">
            <input class="input is-primary" placeholder="<?php echo $placeholder; ?>" <?php echo $search_attrs; ?> />
			<?= elgg_view_icon('search', ['position' => 'left']) ?>
        </p>
    </div>

    <input type="hidden" name="search_type" value="all"/>
    <input type="submit" value="<?php echo elgg_echo('search:go'); ?>" class="is-hidden"/>
</form>
</div>
